...
Windows自动化中一个常见的场景是，将运行时控件的截屏与模型中的保存的对象截屏相比较。这可以通过调用{{book.test_object}}的modelImage方法获得{{book.test_object}}的截屏png图片，以base64字符串数据返回。如果模型中该对象没有对应的截屏图片，则返回null。
